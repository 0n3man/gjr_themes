	...
	function getOptionsSupported() {
		return array(
			...
			gettext('General Thumb Sizes') => array('key' => 'zpmas_thumbsize', 'type' => OPTION_TYPE_CUSTOM, 
				'order' => 2,
				'desc' => gettext('Toggle large or small thumbnails.  This theme does not allow you to set the thumb sizes above for layout reasons. For advanced users, these are set in the functions.php file.')),		
			...
			gettext('Fix the sidebar\'s position?') => array('key' => 'zpmas_fixsidebar', 'type' => OPTION_TYPE_CHECKBOX, 
				'order' => 6,
				'desc' => gettext('Check to fix the sidebar and make it always visible as user scrolls.  If sidebar is too tall for the users viewport, it will revert to static position.')),
			gettext('Logo Image') => array('key' => 'zpmas_logo', 'type' => OPTION_TYPE_TEXTBOX, 
				'order' => 7,
				'desc' => gettext('Enter an image file located in the themes image directory, including extension, to use as an image logo. Or leave blank to use a text representation of your Gallery name.  As an example there is a logo.gif image in the images directory with a height of 83 (must enter height of image as well!).')),
			...
			gettext('Final Image Link Option') => array('key' => 'zpmas_finallink', 'type' => OPTION_TYPE_CUSTOM, 
				'order' => 9,
				'desc' => gettext('Choose the option for the final image link on image.php.  Can either link to full image using standard zenphoto process (with core options), colorbox (if plugin enabled), or no link (default).')),		
